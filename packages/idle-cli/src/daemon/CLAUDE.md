# Idle CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages Idle sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `idle daemon start`

Control Flow:
1. `src/index.ts` receives `daemon start` command
2. Spawns detached process via `spawnIdleCLI(['daemon', 'start-sync'], { detached: true })`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-Idle-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or IDLE_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade idle-coder` occurs:
...
3. If mismatch detected:
   - Spawns new daemon via `spawnIdleCLI(['daemon', 'start'])`
...
1. Backend forwards RPC `spawn-Idle-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached Idle process with `--Idle-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New Idle process:
   - Creates session with backend, receives `IdleSessionId`
   - Calls `notifyDaemonSessionStarted()` to POST to daemon's `/session-started`
5. Daemon updates tracking with `IdleSessionId`, resolves awaiter
6. RPC returns session info to mobile app

### Terminal-Spawned Sessions

User runs `Idle` directly:
1. CLI auto-starts daemon if configured
2. Idle process calls `notifyDaemonSessionStarted()` 
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'Idle directly...'`
4. Session tracked for health monitoring

### Session Termination

Via RPC `stop-session` or health check:
1. `stopSession()` finds session by `IdleSessionId`
...
`Idle doctor` uses `ps aux | grep` to find all Idle processes:
- Production: matches `Idle.mjs`, `idle-coder`, `dist/index.mjs`
...
`Idle doctor clean`:
1. `findRunawayIdleProcesses()` filters for likely orphans
2. `killRunawayIdleProcesses()`:
   - Sends SIGTERM
   - Waits 1 second
...
- Server to Daemon: rpc-request (spawn-Idle-session, stop-session, requestShutdown)
...
socket.emit('machine-update-metadata', {
  ...
  "metadata": "base64(encrypted({
    'host': 'MacBook-Pro.local',
    'platform': 'darwin',
    'IdleCliVersion': '1.0.1',  // version updated
    'homeDir': '/Users/john',
    'idleHomeDir': '/Users/john/.Idle'
  }))",
  "expectedVersion": 1
