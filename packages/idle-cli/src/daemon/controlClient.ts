 ...
 * For instance we are not handling the case where we upgraded idle,
 ...
 * For instance when running `idle daemon status` we can show more information.
 */
...
export async function isDaemonRunningCurrentlyInstalledIdleVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when idle-coder is upgraded globally.
    ...
    /*
    const { spawnIdleCLI } = await import('@/utils/spawnIdleCLI');
    const idleProcess = spawnIdleCLI(['--version'], { stdio: 'pipe' });
    let version: string | null = null;
    idleProcess.stdout?.on('data', (data) => {
      version = data.toString().trim();
    });
    await new Promise(resolve => idleProcess.stdout?.on('close', resolve));
