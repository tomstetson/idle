 ...
 * - connect codex: Store OpenAI API key in Idle cloud
 * - connect claude: Store Anthropic API key in Idle cloud
 * - connect gemini: Store Gemini API key in Idle cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('idle connect')} - Connect AI vendor API keys to Idle cloud

${chalk.bold('Usage:')}
  idle connect codex        Store your Codex API key in Idle cloud
  idle connect claude       Store your Anthropic API key in Idle cloud
  idle connect gemini       Store your Gemini API key in Idle cloud
  idle connect status       Show connection status for all vendors
  idle connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Idle cloud. This enables you to use these services through Idle
...
${chalk.bold('Notes:')} 
  • You must be authenticated with Idle first (run 'idle auth login')
  • API keys are encrypted and stored securely in Idle cloud
  • You can manage your stored keys at idle.northglass.io
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Idle cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with Idle'));
        console.log(chalk.gray('  Please run "idle auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: idle connect <vendor>'));
    console.log(chalk.gray('Example: idle connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with Idle cloud
 * This ensures the Gemini SDK uses the same account as Idle
 */
