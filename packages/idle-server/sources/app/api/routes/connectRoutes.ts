...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect('https://idle.northglass.io?error=invalid_state');
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect('https://idle.northglass.io?error=server_config');
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`https://idle.northglass.io?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect('https://idle.northglass.io?error=github_user_fetch_failed');
            }
            ...
            return reply.redirect(`https://idle.northglass.io?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect('https://idle.northglass.io?error=server_error');
        }
    });
    ...
    app.post('/v1/connect/:vendor/register', {
        ...
        schema: {
            ...
            params: z.object({
                vendor: z.enum(['openai', 'anthropic', 'gemini', 'elevenlabs'])
            })
        }
    }, async (request, reply) => {
        const userId = request.userId;
