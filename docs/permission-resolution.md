...
- Mapping to Claude happens in `packages/idle-cli/src/claude/utils/permissionMode.ts`:
...
`packages/idle-app/sources/sync/storage.ts`
...
`packages/idle-app/sources/sync/persistence.ts`

If draft permission mode is missing:
- Draft default: `default`

### 3) New session UI defaults
`packages/idle-app/sources/app/(app)/new/index.tsx`
`packages/idle-app/sources/components/NewSessionWizard.tsx`
...
`packages/idle-app/sources/sync/sync.ts`
...
`packages/idle-cli/src/claude/runClaude.ts`
`packages/idle-cli/src/claude/utils/permissionMode.ts`
