...
This document describes how to deploy the Idle backend (`packages/idle-server`) and the infrastructure it expects.
...
- `yarn workspace idle-server db` (Postgres in Docker)
- `yarn workspace idle-server redis`
- `yarn workspace idle-server s3` + `s3:init`

Use `.env`/`.env.dev` to load local settings when running `yarn workspace idle-server dev`.

## Implementation references
- Entrypoint: `packages/idle-server/sources/main.ts`
- Dockerfile: `Dockerfile.server`
- Kubernetes manifests: `packages/idle-server/deploy`
- Env usage: `packages/idle-server/sources` (`rg -n "process.env"`)
