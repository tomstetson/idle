...
⚠️ No lint script is defined in `packages/idle-server/package.json`, so linter execution is currently not available.
...
- Update `idle-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `idle-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
