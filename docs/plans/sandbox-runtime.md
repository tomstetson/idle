...
## Overview
Integrate `@anthropic-ai/sandbox-runtime` into idle-cli to sandbox both **Claude Code** and **Codex** sessions with OS-level filesystem and network restrictions. The sandbox wraps agent subprocesses, enforcing configurable restrictions without requiring containers.

Key features:
- **`Idle sandbox configure`** - Interactive CLI wizard (using `inquirer`) to set up sandbox rules
- **`Idle sandbox status`** - Show current sandbox configuration
- **`Idle sandbox disable`** - Turn off sandboxing
- **Automatic enforcement** - Once configured, sandbox wraps both Claude and Codex sessions by default (bypass with `--no-sandbox`)
- **Global config** - Stored in `~/.idle/settings.json` alongside existing settings
- **Dual agent support** - Same sandbox config applies to both Claude Code and Codex

## Context
- **Claude spawn point**: `packages/idle-cli/src/claude/claudeLocal.ts:241` - `spawn('node', [claudeCliPath, ...args], {env, ...})`
- **Codex spawn point**: `packages/idle-cli/src/codex/codexMcpClient.ts:107` - `StdioClientTransport({ command: 'codex', args: ['mcp-server'], env })` which internally calls `cross-spawn('codex', ['mcp-server'])`
- **Config storage**: `packages/idle-cli/src/persistence.ts` - Settings interface + Zod schemas + atomic `updateSettings()`
- **Command dispatch**: `packages/idle-cli/src/index.ts` - manual `if/else if` routing on `args[0]`
- **Existing command pattern**: `packages/idle-cli/src/commands/connect.ts` - exported `handleXxxCommand(args)` functions
...
- [x] Run `yarn add @anthropic-ai/sandbox-runtime inquirer` in `packages/idle-cli`
- [x] Run `yarn add -D @types/inquirer` in `packages/idle-cli`
...
- [x] Create `packages/idle-cli/src/sandbox/config.ts`
...
- [x] Create `packages/idle-cli/src/sandbox/manager.ts`
...
### Task 5: Create `Idle sandbox configure` interactive wizard
- [x] Create `packages/idle-cli/src/commands/sandbox.ts`
...
### Task 6: Implement `Idle sandbox status` and `Idle sandbox disable`
...
### Task 10: Add `Idle sandbox` to help text and polish
- [x] Add `Idle sandbox` to the help text in `index.ts` (alongside `auth`, `connect`, `daemon`, etc.)
...
- [x] Verify `Idle sandbox configure` walks through all questions and saves config (automated command tests)
- [x] Verify `Idle sandbox status` shows current config (automated command tests)
- [x] Verify `Idle sandbox disable` turns off sandbox (automated command tests)
...
- ⚠️ Lint blocker: `packages/idle-cli` has no `eslint.config.*` / `.eslintrc*`, so ESLint 9 cannot run in this package.
...
**Manual verification:**
- Test `Idle sandbox configure` end-to-end on macOS
