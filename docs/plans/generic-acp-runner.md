...
- **Session protocol types** in `@northglass/idle-wire` (`SessionEnvelope`, `createEnvelope()`).
...
**File**: `packages/idle-cli/src/agent/acp/AcpSessionMapper.ts`
...
**Test file**: `packages/idle-cli/src/agent/acp/AcpSessionMapper.test.ts`
...
**File**: `packages/idle-cli/src/agent/acp/runAcp.ts`
...
2. Start Idle MCP server for tool bridge
...
Wire the generic runner into the CLI so users can run `Idle acp gemini` or `Idle acp opencode` or `Idle acp -- custom-agent --flag`.

**Files**:
- `packages/idle-cli/src/index.ts` — add CLI command routing
...
- [ ] Add CLI routing for `Idle acp <agent-name>` and `Idle acp -- <cmd> [args]`
