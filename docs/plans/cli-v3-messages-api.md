# CLI V3 Messages API Migration (idle-cli)

## Overview
Migrate `idle-cli`'s `ApiSessionClient` from Socket.IO-based message read/write to the new v3 HTTP endpoints. The client will:
...
- **Package**: `packages/idle-cli/src/api/apiSession.ts` — `ApiSessionClient` class (EventEmitter)
...
- **InvalidateSync**: Available in `src/utils/sync.ts` (identical to idle-app's version)
...
- ⚠️ `npx eslint` in `packages/idle-cli` currently fails because there is no `eslint.config.(js|mjs|cjs)` in this workspace, so lint verification could not be completed.
