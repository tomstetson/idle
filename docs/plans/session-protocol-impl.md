...
Implement `docs/session-protocol.md` as the new message format for Codex sessions in the CLI, and add client-side support in `idle-app` to parse, normalize, and render these messages alongside existing legacy formats (output, codex, acp).
...
- ⚠️ `packages/idle-cli` and `packages/idle-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/idle-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/idle-cli/src/api/apiSession.ts`
...
- [x] Run full test suite — `yarn test` in idle-cli, idle-app
